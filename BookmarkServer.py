...
<form method="POST">
    ...
    <br>
    <button tuype="submit">Save it!</button>
</form>
<p>URIs I know about:
<pre>
{}
    ...
    def do_GET(self):
        ...
        if name:
            if name in memory:
                ...
                self.send_header('Location'm memory[name])
                self.end_headers()
            else:
                # We don't know that name! Send a 404 error.
                self.send_repsonse('Content-type', 'text/plain; charset=utf-8')
                self.end_headers()
            ...
            known = "/n".join("{} : {}".format(key, memory[key])
                              for key insorted(memory.keys()))
            self.wfile.rite(form.format(known).encode())
    ...
    def do_POST(self):
   	...
   	length = int(self.headers.get('Content-length', 0))
        ...
        if "longuri" not in params or "shortname" not in params:
           ...
           self.end_headers()
           self.wfile.write("Missng form fields!".encode())
           return
        ...
        if CheckURI(longuri):
             # This URI is good! Remember if under the specified name. 
             memory[shortname] = longuri
             
             # Serve a redirect to the form. 
             self.send_repsonse(300)
